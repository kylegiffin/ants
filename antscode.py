...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if isinstance(insect, QueenAnt) and insect.original:
                return
            ...
            if self.ant is insect:
                ...
                if hasattr(self.ant, 'is_container') and self.ant.is_container \
                    ...
                    if isinstance(self.ant, QueenAnt):
                        if self.ant.original:
                            return
                        else:
                            self.ant.is_container = None
...
class Ant(Insect):
    ...
    is_container = False
    can_double = True
    # class attribute, all Ants of the same subclass cost the same to deploy
    # ADD CLASS ATTRIBUTES HERE
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive, min_range=None, max_range=None):
        ...
        current = self.place
        index = 0
        while current.entrance != None or current != hive:
            if len(current.bees) > 0 and self.min_range <= index <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        place_of_bees = list(self.place.bees)
        self.armor = self.armor - amount
        if self.armor <= 0:
            for bee in place_of_bees:
...
# BEGIN Problem 8
class WallAnt(Ant):
    """ Wallant does nothing each turn, has high armor"""
    name = 'WallAnt'
    damage = 0
    food_cost = 4
    implemented = True
...
class BodyguardAnt(Ant):
    ...
    def __init__(self, armor=2):
        Ant.__init__(self, 2)  # The Ant hidden in this bodyguard
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self, armor=1):
        ...
        self.original = QueenAnt.original
        QueenAnt.original = False
        # END Problem 13
            ...
            while location.exit != None:
                if location.ant != None:
                    if location.ant.is_container and location.ant.contained_ant != None:
                        if location.ant.contained_ant.can_double:
                            location.ant.contained_ant.damage *= 2
                            location.ant.contained_ant.can_double = False
                    elif location.ant.can_double:
                        location.ant.damage *= 2
                        location.ant.can_double = False
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 13
        if self.armor - amount <= 0 and self.original:
            bees_win()
        else:
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        if target.hasnotbeenscared:
            target.hasnotbeenscared = False
            apply_effect(make_scare, target, 2)  # back away & stay still for 2 turns
        # END Problem EC
